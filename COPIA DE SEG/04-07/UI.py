...
def addTopIFunction():
	...
	def obtenerValores():
		...
		if fecha=='' or numArt=='' or precioU=='' or cantArt=='':
			messagebox.showwarning("Aviso","Por favor, complete los campos obligatorios!")
			return
		precioT=total()
	...
	ID=Label(addTopI, text='ID',font=('Helvetica','10'))
	ID.grid(column=1,row=1,sticky=N,columnspan=1)
	...
	precio_Total=Label(addTopI, text='Total',font=('Helvetica','10'))
	precio_Total.grid(column=7,row=1,sticky=N)
	
	#Entry:
	IDLabel=Label(addTopI,bd=3,width=4)
	...
	precioTotalEntry=Label(addTopI,bd=3,width=8,font=("Arial",12))
	precioTotalEntry.grid(column=7,row=2)
	
	#BOTON SEND
	sendButton=Button(addTopI,text="Ingresar datos",width=20,command=obtenerValores)
	sendButton.grid(row=2,column=8,padx=10)
